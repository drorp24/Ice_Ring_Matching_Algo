...
def create_deliveries_zones() -> List[Zone]:
    return [
        Zone(create_polygon_2d([create_point_2d(34.9, 31.6), create_point_2d(34.9, 32.4),
                           create_point_2d(35.2, 32.4),
                           create_point_2d(35.2, 31.6)]))

        # create_polygon_2d([create_point_2d(35.05, 31.92), create_point_2d(35.05, 32.018),
        #                    create_point_2d(35.17, 32.018),
        #                    create_point_2d(35.17, 31.92)]),
        # create_polygon_2d(
        #     [create_point_2d(35.05, 32.018), create_point_2d(35.05, 32.2), create_point_2d(35.17, 32.2),
        #      create_point_2d(35.17, 32.018)]),
    ]


def _create_grouped_delivery_request_distribution(center_point: Point2D, sigma_lon: float, sigma_lat: float,
                                                  lowest_priority: int = 10, dr_timewindow: int = 3):
    package_distribution = create_single_package_distribution()
    zero_time = ZERO_TIME
    time_delta_distrib = TimeDeltaDistribution([TimeDeltaExtension(timedelta(hours=dr_timewindow, minutes=0))])
    ...
    # TODO: meital change relative_dr_location_distribution
    # TODO: how to pass zones
    delivery_request_distribution = build_delivery_request_distribution(
        package_type_distribution=package_distribution,
        relative_dr_location_distribution=NormalPointDistribution(center_point, sigma_lon, sigma_lat),
        priority_distribution=PriorityDistribution(list(range(1, lowest_priority))),
...
class MinimumEnd2EndGroupedDrExperiment:
    ...
    def __init__(self, scene: str):
        ...
        if scene == 'north':
            ...
            self.supplier_category_distribution_grouped_dr = SupplierCategoryDistribution(
                zero_time_distribution=DateTimeDistribution([ZERO_TIME]),
                delivery_requests_distribution=_create_grouped_delivery_request_distribution(
                    create_point_2d(35.11, 32.0), 0.03,
                    0.05, 10, 3),
                drone_loading_docks_distribution=DroneLoadingDockDistribution(
                    drone_loading_station_distributions=DroneLoadingStationDistribution(
                        drone_station_locations_distribution=UniformPointInBboxDistribution(35.11,
    ...
    def test_grouped_dr_supplier_category(self):
        start_time = datetime.now()
        empty_drone_delivery_board = _create_empty_drone_delivery_board(amount=20, max_route_time_entire_board=45,
                                                                        velocity_entire_board=10.0)
        print("--- _create_empty_drone_delivery_board run time: %s  ---" % (datetime.now() - start_time))
        ...
        supplier_category = self.supplier_category_distribution_grouped_dr.choose_rand(random=Random(10),
                                                                                       amount={DeliveryRequest: 37,
                                                                                               DroneLoadingDock: 1})
        zones = create_deliveries_zones()
        fully_connected_graph = create_grouped_dr_graph_model(supplier_category, zones)
        print("--- create_fully_connected_graph_model run time: %s  ---" % (datetime.now() - start_time))
        start_time = datetime.now()
...
if __name__ == '__main__':
    ...
    experiment.test_grouped_dr_supplier_category()
