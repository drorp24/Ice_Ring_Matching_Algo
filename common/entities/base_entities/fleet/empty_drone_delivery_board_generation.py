...
def calc_drone_deliveries(drone_set_properties: DroneSetProperties,
                          board_level_properties: BoardLevelProperties) -> [EmptyDroneDelivery]:
    empty_deliveries = []
    ...
    for drone_formation, amount in drone_formations_per_type_amounts.amounts.items():
        for i in range(amount):
            empty_deliveries.append(EmptyDroneDelivery(EntityID(uuid.uuid4()), drone_formation, board_level_properties ))
    return empty_deliveries


def generate_empty_delivery_board(drone_set_properties: [DroneSetProperties],
                                  board_level_properties: BoardLevelProperties = BoardLevelProperties()) -> EmptyDroneDeliveryBoard:
    total_drone_deliveries = []
    for drone_properties in drone_set_properties:
        total_drone_deliveries += calc_drone_deliveries(drone_properties, board_level_properties)
