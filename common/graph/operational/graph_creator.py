...
def _create_directed_from_edges(origin_node: OperationalNode, destinations: List[OperationalNode], factor: float=1.0) -> \
...
def build_time_overlapping_dependent_connected_graph(graph: OperationalGraph, factor: float=1.0):
    ...
    for i, origin_node in enumerate(nodes):
        destinations = list(filter(lambda x: x != origin_node and has_overlapping_time_window(origin_node.internal_node,
                                                                                              x.internal_node),
                                   nodes[i:]))
        edges = _create_directed_from_edges(origin_node, destinations, factor)
        graph.add_operational_edges(edges)
