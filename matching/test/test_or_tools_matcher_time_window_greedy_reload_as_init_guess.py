...
class ORToolsMatcherTimeWindowGreedyReloadAsInitGuessTestCase(TestCase):
    ...
    def test_matcher_when_time_window_greedy_reload_as_init_guess_then_solution_improves(self):
        ...
        matcher_using_init_solution = MatchingMaster(self.tw_match_input)
        tw_actual_delivery_board = tw_matcher.match()
        tw_as_init_guess_actual_delivery_board = matcher_using_init_solution.match_with_time_greedy_as_init_guess()
    ...
    def _create_delivery_requests() -> List[DeliveryRequest]:
        dist = build_delivery_request_distribution(
            relative_pdp_location_distribution=UniformPointInBboxDistribution(min_x=0, max_x=100, min_y=0, max_y=100),
            time_window_distribution=ExactTimeWindowDistribution(24 * [
                TimeWindowExtension(
                    since=ZERO_TIME,
                    until=ZERO_TIME.add_time_delta(TimeDeltaExtension(timedelta(hours=12)))),
            ]),
            package_type_distribution=PackageDistribution({PackageType.LARGE: 1}),
            priority_distribution=ExactPriorityDistribution(list(range(1, 25)))
        )
        return dist.choose_rand(Random(42), amount={DeliveryRequest: 24})
        ...
        dock1 = DroneLoadingDock(EntityID.generate_uuid(),
                                 ...
                                 TimeWindowExtension(
                                     ...
                                     until=ZERO_TIME.add_time_delta(
                                         TimeDeltaExtension(timedelta(hours=12)))))
