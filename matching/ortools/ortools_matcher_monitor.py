...
class ORToolsMatcherMonitor:
    def __init__(self, index_manager: RoutingIndexManager, routing_model: RoutingModel,
                 search_parameters: RoutingSearchParameters, matcher_input: MatcherInput):
    ...
    def add_search_monitor(self):
        ...
        # self._add_best_priority_solution_collector()
        # self._add_unmatched_delivery_requests_collector()
        self._add_last_solution_collector()
        self._add_all_solutions_collector()
    ...
    def monitor_search(self):
        if self.best_solution_collector.SolutionCount() == 0:
            ...
            if self.print_status and self._monitor.num_of_iterations % 10000 == 0:
        ...
        # print(f"iteration {self._monitor.num_of_iterations} : current_objective_value {current_objective_value} last solution objective value {last_objective_value} ")
        # if last_objective_value != best_objective_value:
        #     print(
        #         f"*****iteration {self._monitor.num_of_iterations} : best {best_objective_value} and last {last_objective_value} not equal")
        ...
        if self._monitor.prev_objective_value == best_objective_value and \
                self._monitor.num_of_iterations >= self._monitor_config.min_iterations:
            # todo stop search?
            return True
